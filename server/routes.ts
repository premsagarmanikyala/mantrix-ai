...
app.post('/api/v1/roadmap/merge/preview', async (c) => {
  ...
  const totalDuration = validRoadmaps.reduce((sum, r) => sum + r.estimatedDuration, 0);
  const allBranches = validRoadmaps.flatMap(r => r.branches);
  
  // Simple deduplication by title
  const uniqueBranches = allBranches.filter((branch, index) => 
    allBranches.findIndex(b => b.title === branch.title) === index
  );
  
  const optimizedDuration = uniqueBranches.reduce((sum, b) => sum + b.estimatedDuration, 0);
...
app.post('/api/v1/roadmap/merge', async (c) => {
  const { roadmap_ids, schedule_mode, calendar_view, daily_study_hours } = await c.req.json();
...
app.get('/api/v1/progress/summary', async (c) => {
  ...
  return c.json({
    ...
    currentStreak: 7,
    branchProgress: [
      { branchId: 'branch_1', branchTitle: 'Fundamentals', completed: 5, total: 7, percentage: 71 },
      { branchId: 'branch_2', branchTitle: 'Advanced Topics', completed: 3, total: 8, percentage: 38 },
    ],
  });
});
...
app.post('/api/v1/resume/generate', async (c) => {
  ...
  const resume = await storage.createResume({
    userId: 'demo_user',
    mode,
    content: content || 'Generated resume content based on your learning progress...',
    jobDescription: job_description,
    atsScore: mode === 'analyzer' ? Math.floor(Math.random() * 40) + 60 : undefined,
  });
    ...
    recommendations: [
      {
        ...
        description: 'Enhance your JavaScript skills with advanced design patterns',
        confidence: 0.85,
...
app.get('/api/v1/roadmap/health', (c) => c.json({ status: 'healthy', service: 'roadmap' }));
app.get('/api/v1/roadmap/merge/health', (c) => c.json({ status: 'healthy', service: 'roadmap-merge' }));
app.get('/api/v1/resume/health', (c) => c.json({ status: 'healthy', service: 'resume' }));
app.get('/api/v1/progress/health', (c) => c.json({ status: 'healthy', service: 'progress' }));
